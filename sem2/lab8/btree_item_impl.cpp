...
template <class T>
void BTreeItem<T>::setRoot(std::shared_ptr<T> root)
{
m_figure=root;
}
...
template <typename T>
shared_ptr<BTreeItem<T>> BTreeItem<T>::GoFarLeft(shared_ptr<BTreeItem<T>> current) {
    if (current == nullptr) {
        return nullptr;
    }
...
template <typename T>
shared_ptr<BTreeItem<T>> BTreeItem<T>::GetNext(){
    shared_ptr<BTreeItem<T>> temp;
